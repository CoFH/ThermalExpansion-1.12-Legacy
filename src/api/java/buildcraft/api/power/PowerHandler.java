...
/**
 * The PowerHandler is similar to FluidTank in that it holds your power and allows standardized interaction between machines.
 * <p/>
 ...
 * <p/>
 * If you plan emit power, you need only implement IPowerEmitter. You do not need a PowerHandler. Engines have a PowerHandler because they can also receive
 * power from other Engines.
 * <p/>
...
public final class PowerHandler implements IBatteryProvider {
	...
	public static class PerditionCalculator {
		...
		/**
		 * Apply the perdition algorithm to the current stored energy. This function can only be called once per tick, but it might not be called every tick. It
		 * is triggered by any manipulation of the stored energy.
		 * 
		 * @param powerHandler
	 ...
	 *            This is the minimum about of power that will be accepted by the PowerHandler. This should generally be greater than the activationEnergy if
	 *            you plan to use the doWork() callback. Anything greater than 1 will prevent Redstone Engines from powering this Provider.
	 * @param maxEnergyReceived
	 *            The maximum amount of power accepted by the PowerHandler. This should generally be less than 500. Too low and larger engines will overheat
	 *            while trying to power the machine. Too high, and the engines will never warm up. Greater values also place greater strain on the power net.
	 * @param activationEnergy
	 *            If the stored energy is greater than this value, the doWork() callback is called (once per tick).
	 * @param maxStoredEnergy
	 *            The maximum amount of power this PowerHandler can store. Values tend to range between 100 and 5000. With 1000 and 1500 being common.
	 */
	 ...
	 * <p/>
	 * This function is mostly for legacy implementations. See PerditionCalculator for more complex perdition formulas.
	 * 
	 * @param powerLoss
	 ...
	 * <p/>
	 * If you don't call it, the possibility exists for some weirdness with the perdition algorithm and work callback as its possible they will not be called on
	 * every tick they otherwise would be. You should be able to design around this though if you are aware of the limitations.
	 */
	public void update() {
