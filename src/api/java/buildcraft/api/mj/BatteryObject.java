...
/**
 * A battery object is a wrapper around a battery field in an object. This battery field is of type double, and is the only piece of data specific to this
 * object. Others are class-wide.
 */
public class BatteryObject implements IBatteryIOObject {
	...
	public double getEnergyRequested() {
		...
		try {
			return JavaTools.bounds(batteryData.maxCapacity() - energyStored.getDouble(obj), batteryData.minimumConsumption(),
					batteryData.maxReceivedPerCycle());
	...
	public BatteryObject reconfigure(double maxCapacity, double maxReceivedPerCycle, double minimumConsumption) {
		...
		overrideBattery(maxCapacity, maxReceivedPerCycle, minimumConsumption, batteryData.kind(), batteryData.sides(), batteryData.mode());
		return this;
	}

	public void overrideBattery(final double maxCapacity, final double maxReceivedPerCycle, final double minimumConsumption, final String kind,
			final ForgeDirection[] sides, final IOMode mode) {
