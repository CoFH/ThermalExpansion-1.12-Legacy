...
/**
 * A schematic is a piece of a blueprint. It allows to stock blocks or entities to blueprints, and can have a state that moves from a blueprint referential to a
 * world referential. Although default schematic behavior will be OK for a lot of objects, specific blocks and entities may be associated with a dedicated
 * schematic class, which will be instantiated automatically.
 * 
 * Schematic perform "id translation" in case the block ids between a blueprint and the world installation are different. Mapping is done through the builder
 * context.
 * 
 * Detailed documentation on the schematic behavior can be found on http://www.mod-buildcraft.com/wiki/doku.php?id=builder_support
 * 
...
public abstract class Schematic {
	...
	/**
	 * Blocks are build in various stages, in order to make sure that a block can indeed be placed, and that it's unlikely to disturb other blocks.
	 */
	...
	/**
	 * This is called each time an item matches a requirement. By default, it will increase damage of items that can be damaged by the amount of the
	 * requirement, and remove the intended amount of items that can't be damaged.
	 * 
	 * Client may override this behavior. Note that this subprogram may be called twice with the same parameters, once with a copy of requirements and stack to
	 * check if the entire requirements can be fulfilled, and once with the real inventory. Implementer is responsible for updating req (with the remaining
	 * requirements if any) and slot (after usage).
	 * 
	 * returns what was used (similar to req, but created from slot, so that any NBT based differences are drawn from the correct source)
	 */
	...
	/**
	 * Initializes a schematic for blueprint according to an objet placed on {x, y, z} on the world. For blocks, block and meta fields will be initialized
	 * automatically.
	 */
	...
	/**
	 * Return true if the schematic should not be used at all. This is computed straight after readFromNBT can be used to deactivate schematics in which an
	 * inconsistency is detected. It will be considered as a block of air instead.
	 * 
