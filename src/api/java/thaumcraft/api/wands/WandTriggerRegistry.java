...
/**
 * This class serves a similar function to IWandable in that it allows wands to interact with object in the world. In this case it is most useful for adding
 * interaction with non-mod blocks where you can't control what happens in their code. Example where it is used is in crafting the thaumonomicon from a
...
public class WandTriggerRegistry {
	...
	/**
	 * Registers an action to perform when a casting wand right clicks on a specific block. A manager class needs to be created that implements
	 * IWandTriggerManager.
	 * 
	 * @param manager
	 * @param event
	 *            a logical number that you can use to differentiate different events or actions
	 * @param block
	 * @param meta
	 *            send -1 as a wildcard value for all possible meta values
	 * @param modid
	 *            a unique identifier. It is best to register your own triggers using your mod id to avoid conflicts with mods that register triggers for the
	 *            same block
	 */
	...
	/**
	 * This is called by the onItemUseFirst function in wands. Parameters and return value functions like you would expect for that function.
	 * 
	 * @param world
	 * @param wand
	 * @param player
