...
public class VisNetHandler {
	...
	// / NODE DRAINING
	/**
	 * This method drains vis from a relay or source near the passed in location. The amount received can be less than the amount requested so take that into
	 * account.
	 * 
	 * @param world
	 * @param x
	...
	public static ArrayList<Object[]> findClosestNodes(TileVisNode target, TileVisNode root, ArrayList<Object[]> in) {
		...
		for (WeakReference<TileVisNode> child : root.getChildren()) {
			...
			if (n != null && !n.equals(target) && !n.equals(root)
					&& (target.getAttunement() == -1 || n.getAttunement() == -1 || n.getAttunement() == target.getAttunement())) {
