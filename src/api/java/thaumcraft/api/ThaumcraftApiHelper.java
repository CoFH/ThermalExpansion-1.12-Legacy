...
public class ThaumcraftApiHelper {
	...
	public static boolean itemMatches(ItemStack target, ItemStack input, boolean strict) {
		...
		return (target.getItem() == input.getItem() && ((target.getItemDamage() == OreDictionary.WILDCARD_VALUE && !strict) || target.getItemDamage() == input
				.getItemDamage()));
	}
	...
	/**
	 * Use to subtract vis from a wand for most operations Wands store vis differently so "real" vis costs need to be multiplied by 100 before calling this
	 * method
	 * 
	 * @param wand
	 *            the wand itemstack
	 ...
	 * @param doit
	 *            actually subtract the vis from the wand if true - if false just simulate the result
	 * @param crafting
	 *            is this a crafting operation or not - if false then things like frugal and potency will apply to the costs
	...
	/**
	 * Subtract vis for use by a crafting mechanic. Costs are calculated slightly differently and things like the frugal enchant is ignored Must NOT be
	 * multiplied by 100 - send the actual vis cost
	 * 
	 * @param wand
	 *            the wand itemstack
	 * @param player
	...
	/**
	 * Subtract vis from a wand the player is carrying. Works like consumeVisFromWand in that actual vis costs should be multiplied by 100. The costs are
	 * handled like crafting however and things like frugal don't effect them
	 * 
	 * @param player
	 *            the player using the wand
	 * @param cost
	 ...
	 *            the player using the wand
	 * @param amount
	 *            how much warp to add
	 * @param amount
	 *            how much warp to add. Negative amounts are only valid for temporary warp
	 * @param temporary
	 *            add temporary warp instead of permanent
	 * @param amount
	 *            how much warp to add
	 */
