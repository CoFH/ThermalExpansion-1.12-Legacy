...
public class ThaumcraftApi {
	...
	// Miscellaneous
	/**
	 * Portable Hole Block-id Blacklist. Simply add the block-id's of blocks you don't want the portable hole to go through.
	 */
	...
	/**
	 * This is used to add aspects to entities which you can then scan using a thaumometer. Also used to calculate vis drops from mobs.
	 * 
	 * @param entityName
	 * @param aspects
	 * @param nbt
	 *            you can specify certain nbt keys and their values to differentiate between mobs. <br>
	 ...
	 *            ThaumcraftApi.registerEntityTag("Skeleton", (new AspectList()).add(Aspect.DEATH, 8), new NBTTagByte("SkeletonType",(byte) 1));
	 */
	 ...
	 * 
	 * @param in
	 *            The input of the smelting operation. e.g. new ItemStack(Block.oreGold)
	 * @param out
	 *            The bonus item that can be produced from the smelting operation e.g. new ItemStack(nuggetGold,0,0). Stacksize should be 0 unless you want to
	 *            guarantee that at least 1 item is always produced.
	 */
	public static void addSmeltingBonus(ItemStack in, ItemStack out) {

		smeltingBonus.put(Arrays.asList(Item.getIdFromItem(in.getItem()), in.getItemDamage()), new ItemStack(out.getItem(), 0, out.getItemDamage()));
	}
	 ...
	 * 
	 * @param in
	 *            The ore dictionary input of the smelting operation. e.g. "oreGold"
	 * @param out
	 *            The bonus item that can be produced from the smelting operation e.g. new ItemStack(nuggetGold,0,0). Stacksize should be 0 unless you want to
	 *            guarantee that at least 1 item is always produced.
	 */
	 ...
	 * 
	 * @param in
	 *            The input of the smelting operation. e.g. new ItemStack(oreGold)
	...
	/**
	 * @param research
	 *            the research key required for this recipe to work. Leave blank if it will work without research
	 * @param result
	 *            the recipe output
	 * @param aspects
	 *            the vis cost per aspect.
	 * @param recipe
	 *            The recipe. Format is exactly the same as vanilla recipes. Input itemstacks are NBT sensitive.
	 */
	 ...
	 *            the vis cost per aspect
	 * @param recipe
	 *            The recipe. Format is exactly the same as vanilla shapeless recipes. Input itemstacks are NBT sensitive.
	 */
	 ...
	 *            the research key required for this recipe to work. Leave blank if it will work without research
	 * @param result
	 *            the recipe output. It can either be an itemstack or an nbt compound tag that will be added to the central item
	 * @param instability
	 *            a number that represents the N in 1000 chance for the infusion altar to spawn an instability effect each second while the crafting is in
	 *            progress
	 * @param aspects
	 ...
	 * @param recipe
	 *            An array of items required to craft this. Input itemstacks are NBT sensitive. Infusion crafting components are automatically "fuzzy" and the
	 *            oredict will be checked for possible matches.
	 * 
	 */
	public static InfusionRecipe addInfusionCraftingRecipe(String research, Object result, int instability, AspectList aspects, ItemStack input,
			ItemStack[] recipe) {
	 ...
	 * 
	 */
	public static InfusionEnchantmentRecipe addInfusionEnchantmentRecipe(String research, Enchantment enchantment, int instability, AspectList aspects,
			ItemStack[] recipe) {
	 ...
	 * 
	 * @param item
	 *            the item passed. Pass OreDictionary.WILDCARD_VALUE if all damage values of this item/block should have the same aspects
	 * @param aspects
	 ...
	 * Used to assign apsects to the given item/block. Here is an example of the declaration for cobblestone:
	 * <p>
	 * <i>ThaumcraftApi.registerObjectTag(new ItemStack(Blocks.cobblestone), new int[]{0,1}, (new AspectList()).add(Aspect.ENTROPY, 1).add(Aspect.EARTH,
	 * 1));</i>
	 * 
	 * @param item
	 * @param meta
	 *            A range of meta values if you wish to lump several item meta's together as being the "same" item (i.e. stair orientations)
	 * @param aspects
	 *            A ObjectTags object of the associated aspects
	 */
	...
	/**
	 * Used to assign aspects to the given item/block. Attempts to automatically generate aspect tags by checking registered recipes. Here is an example of the
	 * declaration for pistons:
	 * <p>
	 ...
	 * 
	 * @param item
	 *            , pass OreDictionary.WILDCARD_VALUE to meta if all damage values of this item/block should have the same aspects
	 * @param aspects
	 *            A ObjectTags object of the associated aspects
	 */
	...
	/**
	 * To define mod crops you need to use FMLInterModComms in your @Mod.Init method. There are two 'types' of crops you can add. Standard crops and clickable
	 * crops.
	 * 
	 * Standard crops work like normal vanilla crops - they grow until a certain metadata value is reached and you harvest them by destroying the block and
	 * collecting the blocks. You need to create and ItemStack that tells the golem what block id and metadata represents the crop when fully grown. Sending a
	 * metadata of [OreDictionary.WILDCARD_VALUE] will mean the metadata won't get checked. Example for vanilla wheat:
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestStandardCrop", new ItemStack(Block.crops,1,7));
	 * 
	 * Clickable crops are crops that you right click to gather their bounty instead of destroying them. As for standard crops, you need to create and ItemStack
	 * that tells the golem what block id and metadata represents the crop when fully grown. The golem will trigger the blocks onBlockActivated method. Sending
	 * a metadata of [OreDictionary.WILDCARD_VALUE] will mean the metadata won't get checked. Example (this will technically do nothing since clicking wheat
	 * does nothing, but you get the idea): FMLInterModComms.sendMessage("Thaumcraft", "harvestClickableCrop", new ItemStack(Block.crops,1,7));
	 * 
	 * Stacked crops (like reeds) are crops that you wish the bottom block should remain after harvesting. As for standard crops, you need to create and
	 * ItemStack that tells the golem what block id and metadata represents the crop when fully grown. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will
	 * mean the actualy md won't get checked. If it has the order upgrade it will only harvest if the crop is more than one block high. Example:
	...
	/**
	 * You can define certain ores that will have a chance to produce native clusters via FMLInterModComms in your @Mod.Init method using the "nativeCluster"
	 * string message. The format should be:
	...
	/**
	 * You can blacklist crops that should not be effected by the Lamp of Growth via FMLInterModComms in your @Mod.Init method using the "lampBlacklist"
	 * itemstack message. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will mean the metadata won't get checked. Example for vanilla wheat:
	 * FMLInterModComms.sendMessage("Thaumcraft", "lampBlacklist", new ItemStack(Block.crops,1,OreDictionary.WILDCARD_VALUE));
	 */

	// DIMENSION BLACKLIST ///////////////////////////////////////////////////////////////////////////
	/**
	 * You can blacklist a dimension to not spawn certain thaumcraft features in your @Mod.Init method using the "dimensionBlacklist" string message in the
	 * format "[dimension]:[level]" The level values are as follows: [0] stop all tc spawning and generation [1] allow ore and node generation [2] allow mob
	 * spawning [3] allow ore and node gen + mob spawning Example: FMLInterModComms.sendMessage("Thaumcraft", "dimensionBlacklist", "15:1");
	 */

	// BIOME BLACKLIST ///////////////////////////////////////////////////////////////////////////
	/**
	 * You can blacklist a biome to not spawn certain thaumcraft features in your @Mod.Init method using the "biomeBlacklist" string message in the format
	 * "[biome id]:[level]" The level values are as follows: [0] stop all tc spawning and generation [1] allow ore and node generation [2] allow mob spawning
	 * [3] allow ore and node gen + mob spawning Example: FMLInterModComms.sendMessage("Thaumcraft", "biomeBlacklist", "180:2");
	 */
}
