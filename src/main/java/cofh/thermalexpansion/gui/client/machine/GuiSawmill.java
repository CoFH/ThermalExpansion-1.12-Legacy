...
public class GuiSawmill extends GuiPoweredBase {
	...
	public void initGui() {
		...
		slotTank[0] = (ElementSlotOverlay) addElement(new ElementSlotOverlay(this, 152, 9).setSlotInfo(3, 3, 2).setVisible(false));
		slotTank[1] = (ElementSlotOverlay) addElement(new ElementSlotOverlay(this, 152, 9).setSlotInfo(2, 3, 1).setVisible(false));
	...
	protected void updateElementInformation() {
		...
		slotTank[0].setVisible(myTile.augmentTapper() && baseTile.hasSide(4));
		slotTank[1].setVisible(myTile.augmentTapper() && baseTile.hasSide(3));

		if (!baseTile.hasSide(4)) {
			slotPrimaryOutput[1].slotRender = 2;
			slotSecondaryOutput[1].slotRender = 2;
			slotTank[1].slotRender = 2;
		} else {
			slotPrimaryOutput[1].slotRender = 1;
			slotSecondaryOutput[1].slotRender = 1;
			slotTank[1].slotRender = 1;
		}
