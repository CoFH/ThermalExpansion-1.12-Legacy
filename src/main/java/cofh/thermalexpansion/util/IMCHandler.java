			...
			try {
				...
				switch (operation) {
					...
					case ADD_REFINERY_RECIPE:
						RefineryManager.addRecipe(nbt.getInteger("energy"), FluidStack.loadFluidStackFromNBT(nbt.getCompoundTag("input")), FluidStack.loadFluidStackFromNBT(nbt.getCompoundTag("output")), new ItemStack(nbt.getCompoundTag("secondaryOutput")));
						continue;
